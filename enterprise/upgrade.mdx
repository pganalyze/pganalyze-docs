...
1. During the release upgrade run the migrations with `SKIP_QUERY_STATS_PCO_BACKFILL=1` environment variable set.
2. After the new code is deployed run `Rust::Migrate.migrate('20250902174735_backfill_query_stats_pco')` in a console (`console` command to
    the container / Docker entrypoint).
