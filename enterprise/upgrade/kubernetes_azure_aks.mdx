...
These steps are run on a machine with internet access and Docker installed. Replace `VERSION` with the release 
...
docker tag quay.io/pganalyze/enterprise:VERSION \\
  pganalyzeregistry.azurecr.io/pganalyze-enterprise:VERSION
...
{`az acr import \\
  ...
  --source quay.io/pganalyze/enterprise:VERSION \\
  --image pganalyze-enterprise:VERSION \\
...
Run the following command, replacing `VERSION` with the version you are upgrading to:
...
Run the following command, again replacing `VERSION`:
