...
- pganalyze Enterprise Server now requires Postgres 11 or newer for the database that is used for storing pganalyze internal data. If you are on Postgres 10 or older, we recommend upgrading before migrating to this release.
...
* getIssues API: Allow returning null for issue reference (legacy API) <!-- PR 3084 -->
  - The new "VACUUM: Bloat - VACUUM Blocked by Xmin Horizon" check may in some cases not have any attached issue references, which will cause the legacy "reference" issue field to be null.
