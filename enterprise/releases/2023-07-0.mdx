...
* Index Advisor
  ...
  - Support foreign key constraints that reference tables with indexes using custom functions <!-- PR 2915 -->
...
* Snapshot processing
  ...
  - Re-introduce limit of length on query parsing for very large queries (more than 200,000 characters) <!-- PR 2956 -->
    ...
    - Queries exceeding this length limit will not be considered for Index Advisor, and will show under the "Other" statement type in the Query Performance view
...
* Snapshot processing
    ...
    * Remove automatic restart of snapshot workers every 6 hours <!-- PR 2954 -->
      - In the past this was necessary due to instability of the workers running over a long time - in our testing these problems are resolved, and turning off the restarts will avoid edge cases where the old behaviour caused snapshots to be partially processed
