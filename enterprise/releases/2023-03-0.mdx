...
- Index Advisor improvements
  ...
  - Multiple fixes to query analysis phase (if you've previously seen Index Advisor errors, these may now be resolved)
  - Missing indexes:
...
- Bundled collector upgraded to release [0.49.0](https://github.com/pganalyze/collector/blob/main/CHANGELOG.md#0490------2023-02-27) (from v0.45.1 in the prior release)
  ...
  - See [full collector changelog](https://github.com/pganalyze/collector/blob/main/CHANGELOG.md)
...
- Other improvements
  - Show Amazon RDS / Aurora instance tags on servers in pganalyze
    - For instance with tags applied in AWS (e.g. to indicate which application team owns the instance),
  ...
  - Add `DISABLE_DIRECT_SIGNUP` setting to prevent uninvited users from registering a user account
    ...
    - Note that this setting is not compatible with the `DEFAULT_ORG_ROLE` setting (which when enabled automatically adds new users to the initial organization in pganalyze)
  - Add support for AWS IMDSv2 instance metadata protocol to retrieve credentials for accessing object storage
  - Table columns page: Show NULL % and avg. width (requires [pganalyze.get_column_stats](/docs/install/amazon_rds/01_configure_rds_instance) helper)
