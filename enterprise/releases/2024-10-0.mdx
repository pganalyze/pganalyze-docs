...
- Automated EXPLAIN and EXPLAIN Plans
  - Automated EXPLAIN: Add a better support for handling new lines with the text log format <!-- 3937 -->
...
- Update bundled collector from 0.58.0 to 0.61.0 <!-- XXX -->
  - See the [collector changelog](XXX) for the full details
- Raise connection tracing retention time from 24 hours to 7 days with new packed stats format
...
- Add more time predefined time periods to date picker <!-- 4125 -->
- Display a notice when a server is using an older collector <!-- 4162 -->
...
- Query Details: EXPLAIN Plans tab now respects the date picker selection <!-- 4405 -->
  ...
  - In addition to this, introduce the pagination and also ability to search by the plan fingerprint to the list.
...
- Install Docs: Keep same generated password when moving install between steps <!-- 3933 -->
...
- Update a duplicate snapshot warning logic in Server Settings page <!-- 4240 -->
  - Previously, this was wrongly providing a warning when the test snapshot is executed recently. This is now fixed.
- System Stats: Fix CPU core count calculation <!-- 4244 -->
  - We previously multiplied the CPU logical core count by the number of\nsockets to determine this number, but that's not the commonly expected\nway of calculating this. Also rename the field to "Logical Cores" to\nclarify the meaning.
...
- EXPLAIN: Tweak plan fingerprinting to improve detection of partition indexes <!-- 4304 -->
  - This will result in the same fingerprint for substantially identical\nquery plans that were previously fingerprinted distinctly.
- Update Heroku setup documentation <!-- 4333 -->
