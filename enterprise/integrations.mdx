...
through the following:
 ...
 2. Copy the `app_id` from Integration Setup URL in that same section section
    (note that this is *not* the same id you see in the URL bar),
    and add that as the `PAGERDUTY_APP_ID` environment variable to your pganalyze configuration
 3. Restart the pganalyze Enterprise Server container to apply the new configuration
 4. In pganalyze, go to the Integrations tab in the Settings page and enable
