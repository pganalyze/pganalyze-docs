...
data:
  ...
  PGANALYZE_APP_URL: "https://web.pganalyze.svc.cluster.local:5000" (may not be needed)
---
apiVersion: apps/v1
kind: Deployment
metadata:
...
The pganalyze Collector is not included in the Enterprise Server Slim image, so you will need to run it separately. Select the "Add Server" button at the bottom of the server selection dropdown to get started.

![](add_server_button.png)
