...
* A standalone pganalyze collector running somewhere with access to your PostgreSQL instances (see [Installing the pganalyze Collector](/docs/collector)) 
...
## Step 2: Create an IAM policy that has sufficient access your provider metric APIs

You will need to create a policy that allows the Kubernetes nodes to access the metrics and log data for your PostgreSQL instances. The exact steps depend on your hosting provider,
but an example policy for Amazon RDS is shown in Step 2 of the [AWS Kubernetes setup instructions](/docs/enterprise/setup/kubernetes_amazon_eks#step-2-create-an-iam-policy-that-has-sufficient-access-to-the-amazon-rds-apis).
...
## Step 4: Create the pganalyze Enterprise Slim deployment
 ...
 * a pganalyze pod running as the "worker" application for processing data running as a non-root user
...
data:
  ...
  PGANALYZE_APP_URL: "https://web.pganalyze.svc.cluster.local:5000"
---
apiVersion: apps/v1
kind: Deployment
metadata:
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: web
          image: quay.io/pganalyze/enterprise:v2025.03.4-slim
          args: ["web"]
          envFrom:
            - configMapRef:
                name: pganalyze-config
...
Please see the appropriate setup instructions depending on where your monitored database resides (eg. [self-hosted systems](/docs/install/01_enabling_pg_stat_statements), [Amazon RDS](/docs/install/amazon_rds/01_configure_rds_instance), [Google Cloud SQL](/docs/install/google_cloud_sql/01_create_monitoring_user), [Microsoft Azure](/docs/install/azure_database/01_configure_azure_instance), etc.).
...
You don't need to run anything else on your database server - the pganalyze collector will connect to your database at regular intervals to gather information from PostgreSQL's statistics tables.

### Step 9: Add your first database server to pganalyze

The pganalyze Collector is not included in the Enterprise Server Slim image, so you will need to run it separately. During the setup and configuration process, you will need to set both the
`api_key` and `api_base_url` parameters in your Collector configuration file. 

Please see the instructions for using a separate Collector with pganalyze Enterprise Server [here](/docs/enterprise/setup/separate-collector-install).
