---
title: 'Enterprise Server: Initial Setup on a VM'
backlink_href: /docs/enterprise/setup
backlink_title: 'Enterprise Server: Setup Overview'
---

import ToC from '../../components/Toc'

These are the installation instructions for [pganalyze Enterprise Server](/enterprise-postgres-monitoring) on a VM using Docker or podman.
...
Run the pganalyze Enterprise Server self check, to ensure the configuration is correct:
...
1. Use a [separate collector installation](/docs/enterprise/setup/separate-collector-install). Recommended 
for production deployments with multiple servers, when databases span different networks or regions, 
or when you want the collector closer to the databases it monitors.
2. Use the integrated collector in the pganalyze Enterprise Server image. This is the fastest way to get 
started, especially for evaluations or when monitoring a small number of databases.

To use the integrated collector, follow the instructions shown in pganalyze on the "Add Server" page.
...
Additionally, you can review [all configuration settings](/docs/enterprise/settings) for the pganalyze Enterprise Server container.

## Appendix: How to apply config changes

When you make changes to the pganalyze container configuration in `/etc/pganalyze.env`, make sure to fully restart the container afterwards, like this:
