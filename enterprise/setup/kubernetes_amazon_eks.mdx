...
Before you can add a database to the pganalyze installation, you'll need to enable the `pg_stat_statements` extension on it. This extension is already enabled on most Amazon RDS database servers by default. You can find details in the [Amazon RDS](/docs/install/amazon_rds/00_overview) instructions.
