...
Modify the S3 bucket names (`BUCKETNAME`) to match your setup, and adjust the KMS key details by updating `REGION`/`ACCOUNTID`/`KEYID` to match the key's ARN. And then create the policy:
...
### Step 4: Set up a Kubernetes secret for configuration settings
...
### Step 10: Add your first database server to pganalyze
