...
* Optional: If you use the RDS integration, set up an appropriate IAM role that can be used to download logs and system metrics
  ...
  * You will need both the [collector IAM policy](/docs/install/amazon_rds/03_install_collector_ec2#set-up-iam-policy) as well as the [object storage IAM policy](/docs/enterprise/setup/object-storage-s3)
...
If you'd like to use the Log Insights or Automated EXPLAIN features, or want to run the collector separately from the Enterprise Server container, you should **[set up object storage](/docs/enterprise/setup/object-storage)** before continuing with Step 4.
