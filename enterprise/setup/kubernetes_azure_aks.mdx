...
The goal of this first step is to publish the pganalyze Enterprise container to your private Azure Container Registry, which can be accessed from your Azure AKS cluster.
...
<CodeBlock language="bash">
{`az aks show --resource-group YOUR_RESOURCE_GROUP --name YOUR_AKS_CLUSTER --query oidcIssuerProfile.issuerUrl -o tsv`}
</CodeBlock>
...
#### (Optional) Grant Event Hub permissions for log streaming
If you plan to stream PostgreSQL logs from Azure Event Hub, grant the managed identity the necessary permissions:
...
<CodeBlock language="bash" hideCopy={true}>
