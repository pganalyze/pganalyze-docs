...
If you're using IAM Authentication, you can skip `DB_PASSWORD` above, as there is no password for a monitoring user.
In addition to the above variables, add the following:
...
Instead of specifying `GCP_REGION` like above, you can also update `GCP_CLOUDSQL_INSTANCE_ID` with the shape of `project_id:region:instance_id`.
In that case, you only need to specify `GCP_CLOUDSQL_INSTANCE_ID`, and you can skip `GCP_PROJECT_ID` and `GCP_REGION`.
