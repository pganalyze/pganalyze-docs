...
```
Warning: Limited access to table column statistics detected in database
postgres. Please set up the monitoring helper function
...
```
Your database appears to be missing column stats monitoring helper functions,
which can lead to inaccurate Index Advisor recommendations. Please review the
...
currently running by checking the Debug Info panel on the Settings page for your
server. Then, find the name for the database role that's configured as the
monitoring user in your collector installation (we'll assume "pganalyze" here).
