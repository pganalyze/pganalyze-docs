...
For Heroku Postgres production plans (Standard, Premium, Private, and Shield), you need to create a new credential
(role) for your Postgres instance. This allows you to create a specific `pganalyze` role and apply the same recommended
read-only monitoring privileges used in the other installation guides. More details on managing Heroku Postgres credentials
...
Next, connect to **each database** that you plan to monitor on this server as the default Heroku admin role (NOT the 
...
Finally, use that add-on name once more (`postgresql-objective-11111` in the example) to attach the Heroku Postgres database to the collector. **Be 
