...
Heroku offers to modify several of these settings, but not all of them. Notably,
you cannot update the `auto_explain.log_format` setting with Heroku Postgres
currently. Only JSON format is supported by pganalyze and as text format support
is limited, you may see the errors on some EXPLAIN plans.


You can check out [Heroku documentation](https://devcenter.heroku.com/articles/heroku-postgres-settings#auto-explain)
