...
are only plans that may be more or less appropriate to a given query. A
...
EXPLAIN plans, especially ones gathered with BUFFERS, offer can be compared on
...
about the table, its data, and its indexes gathered by
...
This is usually the most relevant metric overall: this determines query latency,
...
usage. However, this can still be a useful metric if you account for that.


## In pganalyze

In our EXPLAIN comparison view, we simplify plan comparison by showing a diff-like
view of two different EXPLAIN plans:
