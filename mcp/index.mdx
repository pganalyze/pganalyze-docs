...
<DocsEarlyAccessFeature feature="MCP Server" />
...
OAuth is the recommended authentication method for most users. When you first connect, you will be prompted to authorize access through your pganalyze account. During authorization, you can choose between two permission levels:

- **Basic read access** (always granted): Read access to your account data, excluding sensitive information such as query text and EXPLAIN plans.
- **Full read access** (optional): Full read access to your account data, including query text and EXPLAIN plans.
...
API key authentication is useful for non-interactive use cases such as:

- **Automated workflows**: Run scheduled or event-driven agents that query pganalyze without requiring manual OAuth authorization.
- **CI/CD pipelines**: Integrate pganalyze data into your deployment or review processes.
- **Custom integrations**: Build deeper integrations that programmatically access pganalyze data through the MCP server.

## Rate limiting

During the preview, the MCP server is limited to 100 requests per billable server per hour, regardless of authentication method. If you exceed this limit, requests will be rejected until the next hour window. This limit may be adjusted as we learn more about common use cases and usage patterns.
