...
for older rows in order to allow reusing related transaction ids.
...
Because transaction ID wraparound is catastrophic, Postgres has a number of
safeguards that ensure a database is extremely unlikely to actually reach
wraparound. First of all, if the age of the oldest unfrozen transaction ID
...
progress of that VACUUM, delaying bloat cleanup and occupying an autovacuum
worker for an extended period of time, preventing other tables from being
vacuumed.
