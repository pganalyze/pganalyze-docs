...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>[none]</td>
      <td>local address (host:port) to listen on for syslog messages (see below)</td>
    </tr>
  </tbody>
</table>

### Syslog-based Log Insights support

If your database logs are available via syslog, you can use the `db_log_syslog_server` setting to have
the collector listen for syslog messages from a remote server. This should be a local address like
