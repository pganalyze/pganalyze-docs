...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td><code>credential</code>, <code>parsing_error</code>, <code>unidentified</code></td>
...
The `filter_log_secret` setting determines how content coming from logs is filtered:
 ...
 - `statement_parameter`: Bind parameters for a statement (which may contain table data for INSERT statements)
...
And of course, `all` and `none` apply all and none of the above filters, respectively.

The `filter_query_sample` setting determines how content in query samples
(individual executions of a query collected from logs) is filtered:
 - `all`: do not collect any query samples
 - `normalize`: filter query samples to normalize any literal constants used in the query
 - `none`: do not filter query samples at all

The `filter_query_text` setting determines how query text from pg_stat_statements is filtered:
