...
By default the pganalyze collector only filters logs that may contain database secrets:

```
filter_log_secret: credential,parsing_error,unidentified
```

For maximum security (at the cost of less useful data in pganalyze) we recommend setting:
