...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>0</td>
      <td>
        Allow multiple collectors subscribing to the same topic by returning
        a PubSub message that's not for a configured server to the topic (by sending a "Nack"
        response), allowing a different collector to pick it up. After the
