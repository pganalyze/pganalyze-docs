...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>false</td>
      <td>
        Enable to require a WebSocket connection to the pganalyze server, turning off the HTTP(S) fallback. WebSockets are more efficient for submitting snapshots, and are also required for the optional <a href="/docs/workbooks/collector-workflow">
        Workbooks Collector Workflow</a>. A future collector release will change the default of this setting to true.
      </td>
    </tr>
    <tr>
      <td>api_cluster_id (<code>API_CLUSTER_ID</code>)</td>
      <td>auto-detected</td>
      <td>Identity of the cluster the server belongs to. Defaults to cluster ID retrieved from cloud provider APIs (if available), or to the Postgres database system identifier for self-managed servers.
      </td>
    </tr>
    <tr>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>0</td>
      <td>
        Allow multiple collectors to subscribe to the same topic by sending a
        "Nack" response to Pub/Sub messages that don't match a configured
        Postgres server, so they can be redelivered and handled by another collector.
        <br /><br />
        After the
        age limit is reached, messages will always be acknowledged. Keeping the
        limit low is recommended, e.g. 10 minutes (<code>10m</code>), maximum 24 hours (<code>24h</code>).
      </td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td>
        ...
        in the collector configuration file. If using Private Service Connect (PSC), enable `gcp_use_psc` as discussed below.
      </td>
    </tr>
    <tr>
    ...
    <tr>
      <td>gcp_use_psc (<code>GCP_USE_PSC</code>)</td>
      <td>false</td>
      <td>Set to <code>true</code> if the connection is using IAM authentication and the Private Service Connect (PSC) connection should be used rather
      than the private IP address. This requires additional setup ahead of time, specifically the creation of an `A` record as <a href="https://docs.cloud.google.com/alloydb/docs/configure-private-service-connect#configure-dns">described here</a></td>
    </tr>     
  </tbody>
</table>
