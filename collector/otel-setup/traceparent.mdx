...
If you're using the OpenTelemetry SDK with Ruby, the pganalyze collector
integration is best used with `OpenTelemetry::Instrumentation::ActiveRecord` and
`OpenTelemetry::Instrumentation::PG` instrumentation.
With this setup, the EXPLAIN Plan span from the pganalyze collector will be
created as a child trace of the ActiveRecord span, making it easy to relate the
EXPLAIN Plan span to the ActiveRecord code.
However, the span from the pg instrumentation will be created as a sibling span
of the pganalyze collector, which may make it harder to relate them together.
This is due to the limitation of when we can generate a query comment, which is
currently only possible after the ActiveRecord instrumentation, but before the
pg instrumentation.
