...
Setup OpenTelemetry SDK with your application. Read the
...
With above configuration, the query will have the comment like the following:
...
If you are using Rails 6 and below, you can attach SQL comments using a
...
If you're using the OpenTelemetry SDK, this is best to be used with `OpenTelemetry::Instrumentation::ActiveRecord`
and `OpenTelemetry::Instrumentation::PG` instrumentation. Currently, the query
comment is attached by either Rails or marginalia after the ActiveRecord
instrumentation, but before the pg instrumentation.
This makes the span created by the pganalyze collector to have a span from the
ActiveRecord instrumentation as a parent, not the pg one.
This will cause the database/query span (from pg) and EXPLAIN Plan (from the
pganalyze collector) to be a "parallel" relation, which may make it harder to
identify which database span is associated with which EXPLAIN Plan.
