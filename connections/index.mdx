...
the connection that is blocking other queries is also highlighted.
...
cancel or terminate the query C to unblock the query B and A.

This information is available for the historical data too. Let's say the
...
with "Waiting for Lock" state during that time.
If so, you can click the chart to see the snapshot Connection Traces data of
that time with the Lock Monitoring information to see what kind of queries were
running and blocking and/or blocked at that time.
...
have long running queries such as nightly batches, you can either extend the
threshold or add the `/* pganalyze:no-alert */` or `/* pganalyze=no-alert */`
magic comment to avoid getting alerted.
