...
In the schema statistics overview you can find two treemaps, as well as full lists of all the tables and indexes on a given database. The treemap is useful to identify how a specific index or table size relates to the total.

## Table growth over time

![](table-growth.png)

For each table you can track growth over time, understand how a table utilizes TOAST (The Oversized Attribute Storage Technique) for storing large values, and see the aggregate size for all indexes on the table.
...
As queries are received by pganalyze, they are parsed, and an internal association is stored that allows looking up query activity by table. Additionally, each query is broken down into the [individual scans](/docs/indexing-engine/query-to-scan-breakdown).
