...
Different parameter values (or different constant values) can lead to varying performance or even a different plan for a specific query.
...
Query Tuning utilizes a separate user permission, called “Tune Queries”. Only users with
Admin roles will get this permission assigned automatically. This permission allows
users to create workbooks and to use all associated Query Tuning features.

Users without the permission are able to view existing workbooks, including parameter
information, but cannot create new workbooks, variants, or record new plans.
...
automated feature, ensure you're running collector version 0.64.0 or later and follow our
