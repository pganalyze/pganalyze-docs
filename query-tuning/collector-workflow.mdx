...
<CopyCodeBlock content={`CREATE SCHEMA IF NOT EXISTS pganalyze;
...
<CopyCodeBlock content={explainAnalyzeHelper} language="sql" />

To complete the setup, reset the role, and revoke the creation privilege again:

<CopyCodeBlock content={`RESET ROLE;
...
<CopyCodeBlock content={`REVOKE ALL ON FUNCTION dblink(text, text) FROM PUBLIC;
...
<CopyCodeBlock content={`GRANT SELECT ON test TO pganalyze_explain;
GRANT SELECT (col1, col2) ON restricted TO pganalyze_explain;
`} language="sql" />

Or alternatively you can grant access to all tables in a schema:

<CopyCodeBlock content={`GRANT SELECT ON ALL TABLES IN SCHEMA myschema TO pganalyze_explain;`} language="sql" />
...
<CopyCodeBlock content={`DROP FUNCTION pganalyze.explain_analyze(query text, params text[], param_types text[], analyze_flags text[]);`} language="sql" />

And then you can optionally drop the role as well:

<CopyCodeBlock content={`DROP ROLE pganalyze_explain;`} language="sql" />
