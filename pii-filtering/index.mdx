...
service, there are several collector settings that control metadata collection.
...
Note that these queries are already normalized by pg_stat_statements, so
sensitive constants are not a concern here.
...
privacy (at the cost of less useful data in pganalyze), here is the recommended
configuration:
