...
For clusters, the model receives combined scan data from all servers in the cluster. This means 
a single constraint programming model run considers workloads from the primary and all replicas, 
ensuring index recommendations account for the complete query activity across the cluster.
...
[Depending on the workload](/docs/index-advisor/configuration), the Indexing Engine will automatically
