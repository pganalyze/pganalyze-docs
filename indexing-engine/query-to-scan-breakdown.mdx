...
More broadly, each scan consists of:
 ...
 - Optionally, a LIMIT on how much data needs to be returned (Note: not yet
   considered by the Indexing Engine; see [Limitations](/docs/indexing-engine/limitations))
...
For clusters, scans from all servers (primary and replicas) are combined for each table. 
This ensures the Indexing Engine considers the full workload when making index recommendations.

For example, a scan that only occurs on replicas will be included alongside scans from the primary, 
preventing scenarios where an index critical to replica performance would be marked as unused.
