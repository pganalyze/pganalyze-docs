...
unrestricted access to all databases, servers, and [server
groups](/docs/accounts/server_groups), another restrictive permission that
is also assigned will not restrict access for them.
...
You can restrict the view permission to a specific database, server, or
server group. A permission granted on a server group applies to every server and 
...
You can restrict the modify permission to a specific database, server, or server group.
...
You cannot restrict the billing permission to a specific database, server, or server group.

## API

Allows the member to see API keys, typically required for integrating the pganalyze collector on a server.

You cannot restrict the API permission to a specific database, server, or server group.
...
You cannot restrict the manage permission to a specific database, server, or server group.
...
You can restrict the create workbooks permission to a specific database, server, or server group.
