...
As a claim `email` is required, create a new claim with a name `email`. Please note that the namespace field _must be empty_.
For the source attribute, choose `user.email`. If your organization keeps the user's email in the other attribute, pick that one.

![Screenshot of Manage claim](azure_setup_saml_email_claim.png)

Ensure that each user has na email attribute set. This field is empty by default when a new user is created.
You can check this by navigating to **Users**, select a user, then check **Properties** tab.

When these parts are not properly done, you will see the error "SAML integration configuration error - missing email attribute" when trying to single sign-on.
...
You can now assign the Enterprise application to the appropriate users of your team. Going forward your team members can now sign in to pganalyze by using the Enterprise application.
