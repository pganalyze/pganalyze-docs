...
The ORDER BY + LIMIT insight detects when Postgres uses an index primarily for sorting while filtering large numbers of rows, indicating that a different index strategy would be more efficient for the query's WHERE conditions.

## Problem description

Postgres's optimizer sometimes prioritizes ORDER BY optimization over WHERE clause filtering when both are present with LIMIT. This "quickstart" optimization attempts to return results quickly by scanning an index in sort order, but becomes extremely inefficient when matching rows are sparse.
...
If Postgres chooses the `created_at` index for sorting, it may scan millions of rows to find 50 that match `database_id = 123`, instead of using the `database_id` index to filter first.
...
- **Row distribution assumptions**: Postgres assumes matching rows are evenly distributed
...
**How it works:**
- **Disables sort optimization**: Postgres can't use created_at index for sorting `created_at + 0`
...
- Verify index usage change (sort index → filter index)
...
- **Plan stabilization**: Postgres may already be choosing better plans
...
### Monitoring approach
1. **Track plan stability**: Monitor whether Postgres reverts to sort-first strategy
