...
The Inefficient Nested Loops insight detects when PostgreSQL chooses nested loop joins due to row estimation mismatches, often resulting in dramatically slower performance than hash or merge joins would provide.

## Problem description

PostgreSQL's cost-based optimizer sometimes chooses nested loop joins based on incorrect row count estimates. When the actual number of rows significantly exceeds the estimate, nested loops become extremely inefficient compared to hash or merge joins.
...
If PostgreSQL estimates the WHERE clause will return 1 row but it actually returns 10,000 rows, a nested loop join will scan the customers table 10,000 times instead of once.
...
- **Estimated rows**: PostgreSQL planner estimates ≤ 1 row
...
**Benefits:**
- Forces PostgreSQL to materialize the filtered result set
...
- **Plan changes**: PostgreSQL may already be using better plans
...
### Monitoring approach
1. **Track plan changes**: Monitor whether PostgreSQL reverts to nested loops
...
- **[ORDER BY + LIMIT Issues](/docs/query-advisor/insights/order-by-limit)**: Another common estimation-related problem
