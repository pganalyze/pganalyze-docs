...
Detects when an index scan reads many rows that are then discarded by a filter, indicating the chosen index does not cover the query's filter conditions and that a different existing index could return fewer rows.

**[Learn more about Inefficient Index Use →](/docs/query-advisor/insights/inefficient-index-use)**

### Inefficient OR in Join

Detects when a join combines conditions with `OR` where each side filters a different table, forcing the planner to apply the `OR` as a join filter instead of an index scan, and suggests a `UNION` rewrite so each condition can drive an index on its own table.

**[Learn more about Inefficient OR in Join →](/docs/query-advisor/insights/inefficient-or-in-join)**

### Disk Spill Due To Low work_mem

Detects when a Sort or Hash node exceeds `work_mem` and spills intermediate data to temporary files on disk, suggesting an appropriately scoped increase to keep the operation in memory.

**[Learn more about Disk Spill Due To Low work_mem →](/docs/query-advisor/insights/disk-spill-work-mem)**
...
- **[Inefficient Index Use](/docs/query-advisor/insights/inefficient-index-use)** - Spot index scans that discard most rows at the filter
- **[Inefficient OR in Join](/docs/query-advisor/insights/inefficient-or-in-join)** - Split an OR across a join into a UNION so each branch uses an index
- **[Disk Spill Due To Low work_mem](/docs/query-advisor/insights/disk-spill-work-mem)** - Keep Sort and Hash operations in memory
