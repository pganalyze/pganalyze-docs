...
Detects when a join combines conditions with `OR` where each side filters a different table, forcing Postgres to apply the `OR` as a join filter instead of an index scan, and suggests a `UNION` rewrite so each condition can drive an index on its own table.

**[Learn more about Late Join Filter with OR →](/docs/query-advisor/insights/late-join-filter-with-or)**
...
- **[Late Join Filter with OR](/docs/query-advisor/insights/late-join-filter-with-or)** - Split an OR across a join into a UNION so each side uses an index
