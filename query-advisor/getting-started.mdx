...
Query Advisor automatically analyzes PostgreSQL query execution plans to detect optimization opportunities. This guide covers the requirements and setup needed to start using Query Advisor effectively.

## What you get with Query Advisor

Query Advisor provides **continuous, automated query optimization insights** for your PostgreSQL databases:
...
- **Deterministic approach**: Uses purpose-built algorithms based on PostgreSQL planner behavior with no AI hallucinations
...
Query Advisor currently detects and provides solutions for these common PostgreSQL performance issues:

### Inefficient Nested Loops
Detects when PostgreSQL chooses nested loop joins due to row estimation mismatches, suggesting hash or merge join alternatives through query rewrites.

### Wrong Index Due To ORDER BY
Identifies queries where PostgreSQL uses an index only for sorting while filtering large numbers of rows, suggesting modifications to force better index utilization.
...
Query Advisor analyzes PostgreSQL EXPLAIN plans, which can be collected through:
...
To enable continuous Query Advisor analysis, configure PostgreSQL's auto_explain extension:

WIP

### Platform-specific auto_explain setup

The auto_explain configuration process varies by PostgreSQL hosting platform:
...
This setting ensures that EXPLAIN ANALYZE data includes actual row counts, enabling Query Advisor to detect when PostgreSQL's row estimates differ significantly from reality - a key indicator of optimization opportunities.
