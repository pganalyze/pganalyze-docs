...
* **`withRecentData` (boolean)**<br/>Filters for servers that have received data in the last 24 hours (optional), defaults to no filtering - if set to `false` it will filter for all servers that exist, but don't have recent data
