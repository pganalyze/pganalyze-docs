...
read data, but does not directly translate to query runtime. Lower is better. For clusters, this 
includes scans from all server instances in the cluster.
...
improvements are shown as higher impact. For clusters, this accounts for scan 
frequency across all servers.
...
you can view them on the index detail view. Compare the current costs with the estimated new costs. For 
clusters, when viewing the primary server you can see all affected queries for each database hosted by a 
...
the overall weighted cost improvement. For clusters, scan frequency shown on the 
primary server includes activity for both the primary instance and across the whole cluster.

For clusters, the affected queries listed for each scan are labeled with the
server and database they came from, so you can tell whether a given query
originates on the primary or on a replica.
...
current table would change when the index is added. For clusters, affected queries 
may be hosted on any server in the cluster, not just the primary. Each query
lists the server and database it came from.
