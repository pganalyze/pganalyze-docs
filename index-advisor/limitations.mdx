...
The Index Advisor has a number of limitations that affect the types of queries
...
## Limitations
 ...
 - `LIMIT` is not yet considered, causing cost estimates to be unnecessarily
   ...
   calculations
 - Index Advisor currently suggests more indexes than may be reasonable for
   complex workloads
 ...
 - The Index Advisor is based on the Postgres 13.4 planner and may not give
 ...
 - The Index Advisor relies on information collected for the Schema Statistics
   page--if filtering the tables you collect, ensure any tables referenced in
   queries you are interested in are not excluded
 ...
 - PostGIS is not supported, and tables with PostGIS data types will lead to
   errors
